...
# UPDATED: Function to escalate conversation to a human via Chatwoot using API URL and inbox ID from env.
def escalate_to_human(question, customer_id=None):
    import httpx
    chatwoot_api_url = os.getenv("CHATWOOT_API_URL")  # e.g., "https://app.chatwoot.com/api/v1/"
    inbox_id = os.getenv("CHATWOOT_INBOX_ID")           # e.g., "64007"
    token = os.getenv("CHATWOOT_API_KEY")
    endpoint = f"{chatwoot_api_url.rstrip('/')}/inboxes/{inbox_id}/messages"
    print(f"[DEBUG] Escalating to human. Endpoint: {endpoint}, Question: {question}, Customer ID: {customer_id}")
    payload = {
        "content": f"User requested human intervention: {question}",
        "sender": {"id": customer_id} if customer_id else {}
    }
    headers = {"api_access_token": token, "Content-Type": "application/json"}
    try:
         response = httpx.post(endpoint, json=payload, headers=headers, timeout=10)
         response.raise_for_status()
         print(f"[DEBUG] Chatwoot response: {response.status_code} - {response.text}")
    except Exception as e:
         print("Error sending message to Chatwoot:", e)
         print("[DEBUG] Failed Chatwoot API call. Check URL, API key, and payload details.")
    return
...
# UPDATED: In the chat route, add a debug print if escalation is triggered.
@app.route('/api/chat', methods=['POST'])
def chat():
    data     = request.json or {}
    question = data.get('question', '')
    # NEW: Optionally accept a customer identifier (e.g. email or id) from the frontend.
    customer_id = data.get('customer_id', None)
    
    # If user asks to speak with a human, trigger escalation.
    if any(phrase in question.lower() for phrase in ["switch to human", "talk to human", "real human"]):
        print("[DEBUG] Human escalation triggered by user request.")
